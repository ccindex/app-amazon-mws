...
public class AmazonMWS {
   ...
   * 配置amazon数据
   */
  public void requestOrderAmazon(CronOrder cronOrder) throws DatatypeConfigurationException {
    ...
    c.setTime(cronOrder.fireAt);             //设置为上次执行时间
   ...
   * @return
   */
  public List<List<ReportInfo>> responseOrderAmazon() throws Exception{
    ...
    try {
      ...
      if (this.responseList.isSetGetReportListResult()) {
        ...
        if(null!=getReportListResult.getReportInfoList() && getReportListResult.getReportInfoList().size()>0){
          //增加如list 如果还有下一页
          list.add(getReportListResult.getReportInfoList());
          Logger.info(sdf.format(new Date()) + "--getReportList--size:" + getReportListResult.getReportInfoList().size());
        }
        int index=1;
        if(getReportListResult.isHasNext()){
          ...
          while (reportListByNextTokenResult.isHasNext()){
            ...
            list.add(reportListByNextTokenResult.getReportInfoList());
            Logger.info(sdf.format(new Date()) + "--getReportList--size:" + reportListByNextTokenResult.getReportInfoList().size());
          }
        }
        return list;
      }
  ...
  public String generateReportFileName(String reportId, String reportType) {
    ...
    return reportId + "-" + reportType + "-" + calendar.get(Calendar.YEAR) + "-" +(calendar.get(Calendar.MONTH)+1) + "-" +calendar.get(Calendar.DAY_OF_MONTH);
  }
   ...
   */
  public void requestOrderDetailAmazon(ReportInfo reportInfo, String reportFileName, String reportFilePath) throws Exception{
    ...
    try {
      ...
      Logger.error(new Date() + "requestOrderDetailAmazon||errorMsg:" + e.toString());
      throw new Exception(e);
    }
  }
  ...
  public void responseOrderDetailAmazon() {
    try {
      ...
      e.printStackTrace();
      Logger.error(new Date() + "getReport||errorMsg:" + e.toString());
    }
  }
   ...
   * @param cronOrder
   */
  public void dealWithOrderDetailsAmazon(List<List<ReportInfo>> reportInfoList, CronOrder cronOrder) throws Exception{
    ...
    String reportFilePath = cronOrder.accountName+"/"+year + "/" + month + "/"+day + "/";
          ...
          if (ReportList.findByRequestId(reportInfo.getReportId()) == null) {
            ...
            if (reportFile.exists()) {
              ...
              try {
                ...
                S3File s3File = new S3File(reportFileName, "text/plain", me, reportFile.length(), reportFilePath + reportFileName,reportInfo.getReportId(),reportFileName);
                String action="";
  ...
  public void dealWithOrderAmazon(CronOrder cronOrder) throws Exception {
    this.requestOrderAmazon(cronOrder);
    List<List<ReportInfo>> list=this.responseOrderAmazon();
    if(null!=list && list.size()>0){
      this.dealWithOrderDetailsAmazon(list, cronOrder);
    }
  ...
  public Map uploadTrackingNumber(String fileName, String feedType){
    ...
    try {
      ...
      e.printStackTrace();
      Logger.error(new Date() + "uploadTrackingNumber||errorMsg:" + e.toString());
   ...
   * @return
   */
  public void getManageReportScheduleResponse(CronOrder cronOrder){
    ManageReportScheduleRequest requestOne = new ManageReportScheduleRequest();
    requestOne.setReportType(orderType);
    requestOne.setSchedule("_4_HOURS_");
    requestOne.setMerchant(this.merchantId);
    ManageReportScheduleRequest requestTwo = new ManageReportScheduleRequest();
    requestTwo.setReportType(orderUpdateType);
    requestTwo.setSchedule("_4_HOURS_");
    requestTwo.setMerchant(this.merchantId);
    ManageReportScheduleResponse response= null;
    try {
      response = service.manageReportSchedule(requestOne);
      if(response.getManageReportScheduleResult().getCount()>0){
        System.out.println(cronOrder.accountName+"||order is success:"+orderType+"||"+response.getManageReportScheduleResult().getReportScheduleList().get(0).getReportType());
      }
      response = service.manageReportSchedule(requestTwo);
      if(response.getManageReportScheduleResult().getCount()>1){
        System.out.println(cronOrder.accountName+"||order is success:"+orderUpdateType+"||"+response.getManageReportScheduleResult().getReportScheduleList().get(0).getReportType());
      }
    } catch (MarketplaceWebServiceException e) {
      Logger.error(new Date()+"||errorMsg:"+e.toString());
      e.printStackTrace();
    }
